...
if($userid != $serverInfos['user_id']){
    die(header('Location: '.$helper->url().'index/vserver'));
}
...
if(isset($_POST['cores'])){
    ...
    */
    ...
    if($addresses != $serverInfos['addresses']){
        ...
        $i = 1;
        $SQL = $db->prepare("SELECT * FROM `ipv4_pool` WHERE `service_id` IS NULL");
        $SQL->execute();
        if ($SQL->rowCount() > $addresses) {
            while ($row = $SQL->fetch(PDO::FETCH_ASSOC)) {
                if($i <= $addressesCount){
                    $update = $db->prepare("UPDATE `ipv4_pool` SET `service_id`=:service_id,`service_type`=:service_type WHERE `id`=:id");
    ...
    if(empty($error)){
        ...
        if($correctNetwork){
            ...
            $SQL = $db->prepare("INSERT INTO `lxc_servers_tasks`(`service_id`, `task`) VALUES (:service_id, :task)");
        ...
        $SQL = $db->prepare("UPDATE `lxc_servers` SET `cores`=:cores,`memory`=:memory,`disc`=:disc,`addresses`=:addresses,`price`=:price WHERE `id` = :id");
        $SQL->execute(array(":cores" => $cores, ":memory" => $memory, ":disc" => $disk, ":addresses" => $addresses, ":price" => $price_renew, ":id" => $serverInfos['id']));

        if($price > 0){
            $user->removeMoney($price, $userid);
            $user->addOrder($userid,'-'.$price,'LXC V-Server Upgrade');
            echo sendSuccess('Dein vServer wurde geupgraded');
        } else {
            $price = str_replace('-','', $price);
            $user->addMoney($price, $userid);
            $user->addOrder($userid,'-'.$price,'LXC V-Server Downgrade');
