...
if(isset($_POST['buyTraffic'])){
    ...
    if($price > $amount){
        $error = 'Du hast nicht genügend Guthaben';
    }
    ...
    if(empty($error)){
        ...
        $update = $db->prepare("UPDATE `lxc_servers` SET `traffic` = :traffic WHERE `id` = :id");
...
if($available_traffic > $serverInfos['curr_traffic']) {
    ...
    if(isset($_POST['createPreset'])){
        ...
        if(empty($error)){
            $SQL = $db->prepare("INSERT INTO `lxc_servers_command_presets`(`server_id`, `desc`, `command`, `icon`) VALUES (?,?,?,?)");
    ...
    if(isset($_POST['deletePreset'])){
        ...
        $SQL = $db->prepare("SELECT * FROM `lxc_servers_command_presets` WHERE `id` = :id AND `server_id` = :server_id");
        ...
        if(empty($error)){
            $SQL = $db->prepare("DELETE FROM `lxc_servers_command_presets` WHERE `id` = :id");
    ...
    if(isset($_POST['execPreset'])){
        ...
        $SQL2 = $db->prepare("SELECT * FROM `lxc_servers_command_presets` WHERE `id` = :id AND `server_id` = :server_id");
    ...
    if (isset($_POST['reinstallServer'])) {
        ...
        if (empty($error)) {
            ...
            $SQL = $db->prepare("SELECT * FROM `ipv4_pool` WHERE `service_id` = :service_id");
            ...
            $SQL3 = $db->prepare("SELECT * FROM `lxc_servers_nodes` WHERE `id` = :id");
            $SQL3->execute(array(":id" => $serverInfos['node_id']));
            $nodeInfos = $SQL3->fetch(PDO::FETCH_ASSOC);

            $debug = $lxc->recreate($nodeInfos['id'], $serverInfos['id'], $serverOS, $serverInfos['cores'], $serverInfos['memory'], $serverInfos['password'], $serverInfos['disc'], '512', $ip_addrs, $serverInfos['addresses'], $serverInfos['hostname'], $nodeInfos['disc_name']);
            $SQL = $db->prepare("INSERT INTO `lxc_servers_tasks`(`service_id`, `task`) VALUES (:service_id,:debug)");
            $SQL->execute(array(":service_id" => $serverInfos['id'], ":debug" => $debug));

            $update = $db->prepare("UPDATE `lxc_servers` SET `template_id` = :template_id WHERE `id` = :id");
...
//        $root_password = $_POST['root_password'];
        ...
        if (empty($error)) {
            ...
            $lxc->exec($serverInfos['node_id'], $serverInfos['id'], 'chpasswd <<<"root:' . $root_password . '"');
            ...
            $update = $db->prepare("UPDATE `lxc_servers` SET `password` = :password WHERE `id` = :id");
    ...
    if (isset($_POST['installSoftware'])) {
        ...
        $softwareQuery = $db->prepare("SELECT * FROM `lxc_servers_software` WHERE `id` = :id");
...
if(isset($_POST['saveRDNS'])){
    ...
    $SQL = $db->prepare("SELECT * FROM `ipv4_pool` WHERE `service_id` = :service_id AND `ip` = :ip");
    ...
    if(empty($error)){
        ...
        $SQL = $db->prepare("UPDATE `ipv4_pool` SET `rdns` = :rdns WHERE `service_id` = :service_id AND `ip` = :ip");
