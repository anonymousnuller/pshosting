...
$SQLGetServerInfos = $db->prepare("SELECT * FROM `lxc_servers` WHERE `id` = :id AND `type` = 'LXC'");
...
if(isset($_POST['renew'])){
    ...
    if(empty($error)){
        ...
        $SQLGetServerInfos = $db->prepare("UPDATE `lxc_servers` SET `expire_at` = :expire_at, `state` = 'active' WHERE `id` = :id");
        $SQLGetServerInfos -> execute(array(":expire_at" => $expire_at, ":id" => $id));

        $user->removeMoney($price, $userid);
        $user->addOrder($userid,'-'.$price,'LXC V-Server Verlängerung');
