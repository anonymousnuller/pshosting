...
$SQL = $db->prepare("SELECT * FROM `webspaces_packs_normal` WHERE `plesk_id` = :id");
...
if(isset($_POST['renew'])){
    ...
    if(empty($error)){
        ...
        $SQLGetServerInfos = $db->prepare("UPDATE `webspaces` SET `expire_at` = :expire_at, `state` = 'active' WHERE `id` = :id");
        $SQLGetServerInfos -> execute(array(":expire_at" => $expire_at, ":id" => $id));

        $user->removeMoney($price, $userid);
        $user->addOrder($userid,'-'.$price,'Webspace Verlängerung');

        $_SESSION['success_msg'] = 'Dein Webspace wurde verlängert';
        header('Location: '.$helper->url().'manage/webspace/'.$serverInfos['id'] . '/');
