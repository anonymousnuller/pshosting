...
class LXC extends Controller {
    ...
    public function getServerCredentials($nodeID){
        $getServerCredentials = self::db()->prepare("SELECT * FROM `lxc_servers_nodes` WHERE `id` = :nodeID");
    ...
    public function create($nodeID, $serverID, $ostemplate, $cores, $memory, $password, $disc, $swap, array $ip, $addr_count, $hostname, $storage, $network_rate = 60){
        ...
        foreach ($ip as $key => $address) {
            ...
            if($addr_count >= $i+1){
                ...
                $update = self::db()->prepare("UPDATE `ipv4_pool` SET `service_id`=:service_id,`service_type`=:service_type WHERE `id`=:id");
    ...
    public function correctNetwork($nodeID, $serverID)
    {
        ...
        $i = 0;
        $SQL = self::db()->prepare("SELECT * FROM `ipv4_pool` WHERE `service_id` = :service_id");
    ...
    public function getTraffic($nodeID, $serverID){

        $proxmoxVE = new Proxmox(($this->getServerCredentials($nodeID)));
        $proxmoxVE->setResponseType('json');
        $lxctraffic = $proxmoxVE->get($this->getPVE($nodeID).'lxc/'.$serverID.'/status/current');
        $lxctraffic = json_decode($lxctraffic);

        $response = $lxctraffic->data->netin + $lxctraffic->data->netout;

        $total_bytes = $response;
        $endtraffic = round($helper->isa_convert_bytes_to_specified($total_bytes, 'G'));
    ...
    public function getOS($os_id)
    {
        ...
        $SQL = self::db()->prepare("SELECT * FROM `lxc_servers_os` WHERE `template` = :id");
...
//        return $ssh->exec('./lxc-exec '.$vm_id.' '.$command);
        return $ssh->exec('pct exec '. $vm_id . ' ' . $command);
    }
