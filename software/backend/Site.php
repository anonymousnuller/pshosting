...
class Site extends Controller
{
    ...
    public function changeProductDiscount($product_id, $price) {
        $SQL = self::db()->prepare("SELECT * FROM `product_discounts` WHERE `product_id` = :product_id");
        $SQL->execute(array(":product_id" => $product_id));
        $response = $SQL->fetch(PDO::FETCH_ASSOC);

        if($SQL->rowCount() != 0) {
            $SQL2 = self::db()->prepare("UPDATE `product_prices` SET `price` = :price WHERE `name` = :name");
            $SQL2->execute(array(":name" => $response['product_id'], ":price" => $price));
    ...
    public function isTS3PortAviable($node_id, $port){
        $SQL = self::db()->prepare("SELECT * FROM `teamspeak_servers` WHERE `node_id` = :node_id AND `teamspeak_port` = :port AND `deleted_at` IS NULL");
    ...
    public function addOrder($user_id, $amount, $desc) {
        $SQL = self::db()->prepare("INSERT INTO `customer_orders` (`user_id`, `amount`, `desc`) VALUES (:user_id, :amount, :desc)");
    ...
    public function addTransaction($user_id, $gateway, $state, $amount, $desc, $tid){
        $SQL = self::db()->prepare("INSERT INTO `customers_charge_transactions`(`user_id`, `gateway`, `state`, `amount`, `desc`, `tid`) VALUES (:user_id,:gateway,:state,:amount,:desc,:tid)");
        $SQL->execute(array(':user_id' => $user_id, ':gateway' => $gateway, ':state' => $state, ':amount' => $amount, ':desc' => $desc, ':tid' => $tid));
    }

    public function checkTransaction($user_id, $tid, $data) {
        $SQL = self::db()->prepare("SELECT * FROM `customers_charge_transactions` WHERE `user_id` = :user_id AND `tid` = :tid");
        $SQL->execute(array(":user_id" => $user_id, ":tid" => $tid));
    ...
    public function kvmOptionExists($option_id, $value)
    {

        $SQL = self::db()->prepare("SELECT * FROM `kvm_servers_option_entries` WHERE `option_id` = :option_id AND `value` = :value");
        $SQL->execute(array(':option_id' => $option_id, ':value' => $value));
        if($SQL->rowCount() == 1){
            return true;
        } else {
    ...
    public function lxcOptionExists($option_id, $value)
    {

        $SQL = self::db()->prepare("SELECT * FROM `lxc_servers_option_entries` WHERE `option_id` = :option_id AND `value` = :value");
        $SQL->execute(array(':option_id' => $option_id, ':value' => $value));
        if($SQL->rowCount() == 1){
            return true;
        } else {
    ...
    public function lxcGround($option_id, $value, $type){
    ...
    public function kvmGround($option_id, $value, $type){
    ...
    public function validateRootserverOS($os_id)
    {
        ...
        $SQL = self::db()->prepare("SELECT * FROM `kvm_servers_os` WHERE `id` = :id");
    ...
    public function validatevserverOS($os_id)
    {

        $SQL = self::db()->prepare("SELECT * FROM `lxc_servers_os` WHERE `id` = :id");
        $SQL->execute(array(':id' => $os_id));
        if($SQL->rowCount() == 1){
            return true;
        } else {
    ...
    public function getNameOfOS($os_id, $data) {
        $SQL = self::db()->prepare("SELECT * FROM `kvm_servers_os` WHERE `id` = :id OR `prox_id` = :id");
        $SQL->execute(array(":id" => $os_id));
        if($SQL->rowCount() == 1) {
            $response = $SQL->fetch(PDO::FETCH_ASSOC);
            return $response[$data];
    ...
    public function getNodeName($node_id) {
        $SQL = self::db()->prepare("SELECT * FROM `kvm_servers_nodes` WHERE `id` = :id");
    ...
    public function validateKVMOS($os_id, $type)
    {

        $SQL = self::db()->prepare("SELECT * FROM `kvm_servers_os` WHERE `id` = :id AND `type` = :type");
        $SQL->execute(array(':id' => $os_id, ":type" => $type));
    ...
    public function getLastTicketReactionID($ticket_id, $userid) {
        $SQL = self::db()->prepare("SELECT * FROM `support_tickets_messages` WHERE `ticket_id` = :ticket_id AND `writer_id` = :writer_id ORDER BY `id` DESC LIMIT 1");
        $SQL->execute(array(":ticket_id" => $ticket_id, ":writer_id" => $userid));
        if($SQL->rowCount() == 0){
            return '100';
        }
        $response = $SQL->fetch(PDO::FETCH_ASSOC);
    ...
    public function getLastTicketReaction($ticket_id, $userid) {
        $SQL = self::db()->prepare("SELECT * FROM `support_tickets_messages` WHERE `ticket_id` = :ticket_id AND `writer_id` = :writer_id ORDER BY `id` DESC LIMIT 1");
        $SQL->execute(array(":ticket_id" => $ticket_id, ":writer_id" => $userid));
        if($SQL->rowCount() == 0){
            return '100';
    ...
    {
        $SQL = self::db()->prepare("SELECT * FROM `kvm_servers` ORDER BY `id` DESC LIMIT 1;");
        $SQL->execute();
        if($SQL->rowCount() == 0){
            return '100';
        }
    ...
    public function getLastLXCID()
    {
        $SQL = self::db()->prepare("SELECT * FROM `lxc_servers` ORDER BY `id` DESC LIMIT 1;");
        $SQL->execute();
        if($SQL->rowCount() == 0){
            return '900';
        }
    ...
    {
        $SQLSelectServers = self::db()->prepare("SELECT * FROM `ipv4_pool` WHERE `service_id` = :service_id");
    ...
    {
        $SQLSelectServers = self::db()->prepare("SELECT * FROM `ipv4_pool` WHERE `service_id` = :service_id ORDER BY `id` DESC LIMIT 1;");
        $SQLSelectServers->execute(array(":service_id" => $service_id));
        if ($SQLSelectServers->rowCount() != 0) {
            while ($row = $SQLSelectServers->fetch(PDO::FETCH_ASSOC)) {
                return $row[$type];
