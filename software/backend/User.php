...
class User extends Controller {
    ...
    public function getRole($data) {
        $SQL = self::db()->prepare("SELECT * FROM `customers` WHERE `email` = :email OR `id` = :id OR `username` = :username");
    ...
    public function resetPW($data, $password) {
        ...
        $SQL = self::db()->prepare("UPDATE `customers` SET `password` = :password WHERE `username` = :username OR `email` = :email");
    ...
    {
        $SQL = self::db()->prepare('SELECT * FROM `customers` WHERE `support_pin` = :s_pin');
