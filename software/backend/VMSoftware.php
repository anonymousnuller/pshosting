...
class VMSoftware extends Controller
{
    ...
    {
        $SQL = self::db()->prepare("INSERT INTO `lxc_servers_software_tasks`(`vm_id`, `type`) VALUES (?, ?)");
    ...
    public function getOpenInstalls($vm_id)
    {
        ...
        $SQL = self::db()->prepare("SELECT * FROM `lxc_servers_software_tasks` WHERE `created_at` > :dateMinute AND `vm_id` = :vm_id");
