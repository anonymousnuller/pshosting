<?php
$currPage = 'customer_KVM Rootserver Übersicht';
...
if(isset($_POST['saveCustomName'])){
    ...
    if(empty($error)){
        ...
        $SQL = $db->prepare("UPDATE `kvm_servers` SET `custom_name` = :custom_name WHERE `id` = :id");
        ...
        <div id="kt_content_container" class="container-xxl">
            <div class="container">
                <div class="row">
                    ...
                    <?php
                    $SQL = $db->prepare("SELECT * FROM `kvm_servers` WHERE `user_id` = :user_id AND `deleted_at` IS NULL ORDER BY `id` DESC");
                    ...
                    if ($SQL->rowCount() != 0) {
                        while ($row = $SQL -> fetch(PDO::FETCH_ASSOC)){
                            ...
                            if(!is_null($row['job_id'])){
                                $job_infos = $hosterapi->getJob($row['job_id']);
                                if($job_infos->result->status == 'finished'){
                                    $update = $db->prepare("UPDATE `kvm_servers` SET `hosterapi_id` = :hosterapi_id, `job_id` = NULL, `password` = :password WHERE `id` = :id");
                                    $update->execute(array(":hosterapi_id" => json_encode($job_infos), ":password" => $job_infos->result->output->password, ":id" => $row['id']));
                                } else {
                                            ...
                                            <div class="col">
                                                <h3 class="mb-0">
                                                    ...
                                                    <?php if(is_null($row['custom_name'])){ ?>
                                                        <b>KVM Rootserver</b> #<?= $row['id']; ?> <span data-toggle="tooltip" title="Eigenen Produkt-Namen setzen"><i style="cursor:pointer;" data-toggle="modal" data-target="#productModal<?= $row['id']; ?>" class="far fa-edit"></i></span>
                                                    <?php } else { ?>
