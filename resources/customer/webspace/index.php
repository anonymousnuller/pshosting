<?php
$currPage = 'customer_Webspace Übersicht';
...
if(isset($_POST['saveCustomName'])){
    ...
    if(empty($error)){
        ...
        $SQL = $db->prepare("UPDATE `webspaces` SET `custom_name` = :custom_name WHERE `id` = :id");
    ...
    <div class="post d-flex flex-column-fluid" id="kt_post">
        ...
        <div id="kt_content_container" class="container-xxl">
            ...
            <div class="row">
                ...
                <?php
                $SQL = $db->prepare("SELECT * FROM `webspaces` WHERE `user_id` = :user_id AND `deleted_at` IS NULL ORDER BY `id` DESC");
                $SQL->execute(array(":user_id" => $userid));
                if ($SQL->rowCount() != 0) {
                    while ($row = $SQL -> fetch(PDO::FETCH_ASSOC)){
                                ...
                                <div class="modal-content">
                                    <form method="post">
                                        ...
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-outline-primary text-uppercase font-weight-bolder" data-bs-dismiss="modal"><i class="fas fa-ban"></i> Abbrechen</button>
                        ...
                        <div class="col-12 col-xl-3">
                            <div class="card text-center shadow mb-5">
                                <div class="card-header align-items-center">
                                    <h3 class="card-title text-center mb-0" style="align-content: center;">
                                        <?php if(is_null($row['custom_name'])){ ?>
                                            <b>Webspace</b> #<?= $row['id']; ?> &nbsp; <span data-bs-toggle="tooltip" title="Eigenen Produkt-Namen setzen"><i style="cursor:pointer;" data-bs-toggle="modal" data-bs-target="#webspaceModal<?= $row['id']; ?>" class="far fa-edit"></i></span>
                                        <?php } else { ?>
                                            <?= $helper->xssFix($row['custom_name']); ?> &nbsp; <span data-bs-toggle="tooltip" title="Eigenen Produkt-Namen setzen"><i style="cursor:pointer;" data-bs-toggle="modal" data-bs-target="#webspaceModal<?= $row['id']; ?>" class="far fa-edit"></i></span>
                                        <?php } ?>
                                    </h3>
                                </div>
                                <div class="card-body">
                                    ...
                                    <div class="row align-items-center">
                                        <div class="col">
                                            <h4 class="mb-lg-4">
                                                <b>Domain</b><br>
                                                <span class="badge badge-success" style="font-size: 70%; text-align: center;"><?= $row['domainName']; ?></span>
                                            </h4>
                                        </div>
                                    </div>
                    ...
                    <?php $items++; } } if($items == 0){ ?>
                        <div class="col-md-12">
                            ...
                            <?php } ?>
                                ...
                                <br>
                                <p>
                                    <a href="<?= env('URL'); ?>order/webspace/" class="btn btn-outline-primary text-uppercase font-weight-bolder">Jetzt bestellen</a>
                                </p>
                            </div>
                        </div>
                    <?php } ?>
