<?php
$currPage = 'customer_LXC V-Server';
...
if(isset($_POST['saveCustomName'])){
    ...
    if(empty($error)){
        ...
        $SQL = $db->prepare("UPDATE `lxc_servers` SET `custom_name` = :custom_name WHERE `id` = :id");
        ...
        <div id="kt_content_container" class="container-xxl">
            <div class="container">
                <div class="row">
                    ...
                    <?php
                    $SQL = $db->prepare("SELECT * FROM `lxc_servers` WHERE `user_id` = :user_id AND `deleted_at` IS NULL AND `type` = 'LXC' ORDER BY `id` DESC");
                    $SQL->execute(array(":user_id" => $userid));
                    if ($SQL->rowCount() != 0) {
                        while ($row = $SQL -> fetch(PDO::FETCH_ASSOC)){
                            ...
                            <div class="col-md-12">
                                ...
                                <?php } ?>
                                    <h1 class="alert-heading">
                                        <br>
                                        Du hast aktuell noch keinen LXC V-Server 😲
                                    </h1>
                                    <br>
                                    <h4>
                                        Miete dir jetzt einen Prepaid LXC V-Server bei uns!
                                    </h4>
                                    <br>
                                    <p>
                                        <a href="<?= env('URL'); ?>order/vserver" class="btn btn-outline-primary text-uppercase font-weight-bolder">Jetzt bestellen</a>
                                    </p>
                                </div>
                            </div>
                        <?php } ?>
